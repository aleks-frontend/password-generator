$(document).on('knack-view-render.view_1', function (event, view, data) {
    ...
    // Helper function for setting the password password input field
    function setPasswordToInput() {
      $passwordInput.val(getNewPassword());
    }
    
    // Helper function for copying the password to clipboard
    function copyPasswordToClipboard() {
      $passwordInput.select();
      document.execCommand('copy');
    }
    
    // Helper function for setting and copying the password
    function setAndCopyPassword() {
      setPasswordToInput();
    ...
    $passwordComplexity.on('change', function (e) { setAndCopyPassword() });
    $passwordLength.on('change', function (e) { setAndCopyPassword() });
  });
